...
privileged aspect AdvogadoController_Roo_Controller {
    ...
    @Autowired
    ClienteService AdvogadoController.clienteService;
    ...
    public String AdvogadoController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("advogadoes", Advogado.findAdvogadoEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) advogadoService.countAllAdvogadoes() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("advogadoes", Advogado.findAllAdvogadoes(sortFieldName, sortOrder));
        }
        addDateTimeFormatPatterns(uiModel);
    ...
    void AdvogadoController.addDateTimeFormatPatterns(Model uiModel) {
        uiModel.addAttribute("advogado_datanascimento_date_format", DateTimeFormat.patternForStyle("M-", LocaleContextHolder.getLocale()));
    }
